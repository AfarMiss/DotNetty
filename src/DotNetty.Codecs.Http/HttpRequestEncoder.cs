    ...
    public class HttpRequestEncoder : HttpObjectEncoder<IHttpRequest>
    {
        ...
        protected internal override void EncodeInitialLine(IByteBuffer buf, IHttpRequest request)
        {
            ...
            else
            {
                ...
                throw new NotImplementedException();
                // buf.Write<byte>(HorizontalSpace).WriteCharSequence(uriCharSequence, Encoding.UTF8);
                if (needSlash)
                {
                    // write "/ " after uri
                    buf.Write<short>(SlashAndSpaceShort);
                }
                else
                {
                    buf.Write<byte>(HorizontalSpace);
                }
            }

            request.ProtocolVersion.Encode(buf);
            buf.Write<short>(CrlfShort);
        }
    }
}
