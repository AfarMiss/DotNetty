...
namespace DotNetty.Buffers
{
    public static class ByteBufferUtil
    {
        ...
        public static bool IsText(IByteBuffer buf, int index, int length, Encoding encoding)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        IByteBuffer heapBuffer = ByteBuffer.Allocator.Buffer(length);
                        try
                        {
        ...
        static bool IsAscii(IByteBuffer buf, int index, int length) => ForEachByte(buf, index, length, AsciiByteProcessor) == -1;
