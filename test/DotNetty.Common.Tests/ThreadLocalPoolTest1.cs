...
namespace DotNetty.Common.Tests
{
    ...
    public class ThreadLocalPoolTest1
    {
        ...
        public void ThreadCanBeCollectedEvenIfHandledObjectIsReferencedTest()
        {
            ...
            // IRecycleHandle<HandledObject> recycleHandle = null;
            ...
            //         HandledObject data = pool.Acquire(out recycleHandle);
            ...
            var fff = new Dictionary<int, WeakReference<ThreadLocalMap>>();
            fff1 = new Dictionary<int, WeakReference<IRecycleHandle<HandledObject>>>();
            for (int i = 0; i < 100; i++)
            {
                var thread12 = new Thread(() =>
                {
                    fff.Add(i, new WeakReference<ThreadLocalMap>(ThreadLocalMap.Get()));
                    HandledObject data = pool.Acquire(out var fRecycleHandle);
                    fff1.Add(i, new WeakReference<IRecycleHandle<HandledObject>>(fRecycleHandle));
                });
                thread12.Start();
                thread12.Join();
            }

            // var weakReferences = fff.Values.Where(weakReference => weakReference.TryGetTarget(out _));
            // var enumerable = weakReferences.ToList();
            // var references1 = enumerable.Where(weakReference => weakReference.TryGetTarget(out _)).ToList();
            // references1.Clear();
            fff.Clear();
            fff = null;
            for (int i = 0; i < 5; ++i)
            ...
            var recycleHandles = this.fff1;
            // var tryGetTarget = Volatile.Read(ref threadRef)?.TryGetTarget(out _);
            // Assert.False(tryGetTarget);
